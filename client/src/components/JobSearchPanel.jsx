    ...
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E5E7EB", padding: "16px 18px", display: "flex", flexDirection: "column", gap: 8, opacity: applied ? 0.65 : 1 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", flex: 1 }}>
          <input type="checkbox" checked={!!applied} onChange={(e) => onToggleApplied(index, e.target.checked)} style={{ marginTop: 4, width: 16, height: 16, cursor: "pointer", flexShrink: 0 }} />
          <div>
            <div style={{ fontWeight: 700, fontSize: "0.95rem", color: "#111827", textDecoration: applied ? "line-through" : "none" }}>{job.title || "Role — see employer"}</div>
      ...
      {isManual ? (
        <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 8, padding: "8px 12px", fontSize: "0.8rem", color: "#92400E" }}>
          ⚠️ Manual check needed — {job.manual_check_reason || "couldn't read their careers page automatically"}.
...
export default function JobSearchPanel({ country, cfg, cvs, activeCvId, employerLists }) {
  ...
  useEffect(() => { refreshSaved(); setCurrentResult(null); setSelectedListId(""); }, [country, refreshSaved]);
  useEffect(() => { if (employerLists.length && !employerLists.find((l) => l.id === selectedListId)) setSelectedListId(employerLists[0]?.id || ""); }, [employerLists, selectedListId]);
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E5E7EB", padding: "18px 20px" }}>
      <div style={{ fontWeight: 700, fontSize: "0.95rem", color: "#111827", marginBottom: 4 }}>✈️ Visa-Sponsored Job Search — {cfg.label}</div>
      <div style={{ fontSize: "0.76rem", color: "#9CA3AF", marginBottom: 14 }}>Searches each employer's careers page, filters for sponsored roles, and scores matches against your CV.</div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
        ...
        <div style={{ flex: "1 1 200px", fontSize: "0.83rem", color: activeCv ? "#111827" : "#DC2626" }}>
          CV: <strong>{activeCv ? activeCv.title : "None selected — pick one above"}</strong>
        </div>
        <button onClick={handleSearch} disabled={searching || !selectedListId || !activeCvId} style={{
        ...
        }}>{searching ? "Searching live…" : "🔍 Search Jobs"}</button>
      </div>

      {error && <div style={{ color: "#DC2626", fontSize: "0.8rem", marginBottom: 10 }}>{error}</div>}
      {searching && <div style={{ textAlign: "center", padding: "30px 0", color: "#6B7280", fontSize: "0.85rem" }}>Searching career pages and scoring matches — this takes 20-60 seconds…</div>}

      {savedResultSets.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: "0.78rem", fontWeight: 700, color: "#6B7280", marginBottom: 6 }}>Saved searches ({savedResultSets.length}/4):</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {savedResultSets.map((s) => (
              <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 20, border: `1px solid ${currentResult?.id === s.id ? cfg.color : "#E5E7EB"}`, background: currentResult?.id === s.id ? cfg.accentLight : "#fff", fontSize: "0.76rem" }}>
                <span style={{ cursor: "pointer" }} onClick={() => api.getSearchResult(country, s.id).then(setCurrentResult)}>{s.cv_title} · {new Date(s.created_at).toLocaleDateString()} ({s.job_count})</span>
                <span style={{ color: "#DC2626", fontWeight: 700, cursor: "pointer" }} onClick={() => { if(confirm("Delete?")) api.deleteSearchResult(country, s.id).then(refreshSaved); }}>✕</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {currentResult && !searching && (
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
            ...
            <button
              onClick={() => {
                const header = "Employer,Job Title,Location,Match %,Sponsorship,URL,Missing From CV\n";
                const rows = currentResult.results.map((j) =>
                  `"${(j.employer||"").replace(/"/g,'""')}","${(j.title||"").replace(/"/g,'""')}","${(j.location||"").replace(/"/g,'""')}","${j.match_percentage ?? "N/A"}","${j.sponsorship_mentioned ? "Yes" : "No"}","${(j.url||"").replace(/"/g,'""')}","${(j.missing_from_cv||[]).join("; ").replace(/"/g,'""')}"`
                ).join("\n");
                const blob = new Blob([header + rows], { type: "text/csv;charset=utf-8;" });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url;
                a.download = `job-results-${currentResult.cv_title}-${new Date(currentResult.created_at).toLocaleDateString()}.csv`;
                a.click();
      ...
      {!currentResult && !searching && (
        <div style={{ fontSize: "0.85rem", color: "#9CA3AF", textAlign: "center", padding: "20px 0" }}>Select an employer list and CV above, then click Search Jobs.</div>
      )}
    </div>
  );
}
